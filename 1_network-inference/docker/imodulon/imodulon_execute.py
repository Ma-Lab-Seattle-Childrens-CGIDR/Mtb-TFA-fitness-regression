...
# and https://github.com/SBRG/pymodulon/blob/master/src/pymodulon/core.py
...
def main(expression_file, iterations=1, tolerance=1e-7, out_dir='', distance=0.1,
		minimum_fraction=0.5, timeout_seconds=-1, dimensionality='scan', dim_begin=20, dim_end=0,
		dim_step=0, finalize=True):
		...
		for n_dimensions in dimension_counts:
			...
			for i in range(iterations):
				if timeout_seconds <= 0:
					...
					process = Process(
						target=run_ica, args=(data_df, tolerance, n_dimensions, tmp_dir, i))
					process.start()
					process.join(timeout_seconds) # wait timeout_seconds or until run_ica is done

					if process.is_alive():
						print(f'Killing round {i+1} of ICA due to overlong execution time',
							f'(>{timeout_seconds}s)')
						process.kill()
						process.join()
	...
	if not finalize == False: # truthy check here would suffice, but check explicitly for clarity
		get_dimension(out_dir, distance, minimum_fraction)
...
def run_ica(data_df, tolerance, n_dimensions, tmp_dir, i):
	...
	ica = FastICA(whiten='arbitrary-variance', max_iter=int(1e10), tol=tolerance,
...
def get_dimension(out_dir, distance, minimum_fraction):
	print('Computing optimal set of independent components')

	def check_mat(dim, mat='M'):
		filename = os.path.join(out_dir, 'ica_runs', str(dim), mat + '.csv')
		if not os.path.exists(filename):
			try:
				tmp_dir = os.path.join(out_dir, 'ica_runs', str(dim), 'tmp')
				iterations = max(
					(int(sub)+1 for file in os.listdir(tmp_dir) \
						for sub in file.split('_') if sub.isdigit()),
					default=0
				)
				print(f'Aggregating {iterations} non-clustered iterations for dimension {dim}')
				compute_distances(tmp_dir, iterations)
				cluster_components(
					os.path.join(tmp_dir, '..'), tmp_dir, iterations, distance, minimum_fraction)
			except:
				print(f'Skipping failed dimension {dim}')
				return False
		return True

	def load_mat(dim, mat, failsafe=True):
		df = pd.read_csv(
	...
	dims = [dim for dim in dims if check_mat(dim)]
...
if __name__ == '__main__':
	...
	parser.add_argument('-t', '--tolerance',
		...
		help=('ICA convergence tolerance. Larger values run faster but provide less accurate '
			'components.'))
	parser.add_argument('-s', '--timeout_seconds',
		default=-1,
		type=int,
		help=('Timeout (in seconds) for each iteration of ICA. ICA runs that take longer than this '
			'will be killed, and processing will continue. The total number completed runs will be '
